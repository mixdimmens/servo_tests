...
for row in range(len(values)):
    ...
    # plt.title('inside: {}, outside: {}'.format(flip_ang(values[row][1]), flip_ang(values[row][2])))
    plt.title('move No. {}, direction: {}'.format(row, direction))
    # plt.show()
    plot_num +=1
    plot_filename = plot_title + str(plot_num)
    plt.savefig(plot_filename, transparent= True, pad_inches= 1)
    plt.clf()